...
resource "aws_cloudfront_distribution" "machbio_distribution" {
  ...
  viewer_certificate {
    acm_certificate_arn = aws_acm_certificate.machbio_cert.arn
    ssl_support_method  = "sni-only"
  }
}

resource "aws_acm_certificate" "machbio_cert" {
  private_key       = var.machbio_private_key_pem
  certificate_body  = var.machbio_certificate_pem
  certificate_chain = var.machbio_issuer_pem
