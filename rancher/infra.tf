...
resource "digitalocean_ssh_key" "quickstart_ssh_key" {
  ...
  public_key = "${tls_private_key.do.public_key_openssh}"
}
...
resource "digitalocean_droplet" "rancher_server" {
  ...
  user_data = templatefile("${path.module}/cloud-common/files/userdata_rancher_server.template", {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      ...
      private_key = tls_private_key.do.private_key_pem
    }
  }
}
...
module "rancher_common" {
  ...
  ssh_key_file_name      = tls_private_key.do.private_key_pem
...
resource "digitalocean_droplet" "quickstart_node" {
  ...
  user_data = templatefile("${path.module}/cloud-common/files/userdata_quickstart_node.template", {
    docker_version   = var.docker_version
    username         = local.node_username
