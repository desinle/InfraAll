...
resource "tls_private_key" "do_key" {
  algorithm = "RSA"
  rsa_bits  = 4096
}
...
resource "digitalocean_ssh_key" "quickstart_ssh_key" {
  ...
  public_key = tls_private_key.do_key.public_key_openssh
}
...
resource "digitalocean_droplet" "rancher_server" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      ...
      private_key = tls_private_key.do_key.private_key_pem
    }
  }
}
...
module "rancher_common" {
  ...
  ssh_key_file_name      = tls_private_key.do_key.private_key_pem
